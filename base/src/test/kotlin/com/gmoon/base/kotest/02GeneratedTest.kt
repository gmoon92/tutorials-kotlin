...
class `02GeneratorsTest` : FunSpec({
    ...
    context("Arbitrary 임의 generator") {
        test("Arb.int 기본 edge case 포함") {
            ...
            values shouldContainAll listOf(0, 1, -1, Int.MAX_VALUE, Int.MIN_VALUE)
        }
        ...
        test("Arb.string edge case (empty string, min length 등)") {
            val values = Arb.string()
                .samples(RandomSource.seeded(1236L))
                .take(100)
                .map { it.value }
        ...
        test("Arb.list(Int) edge case (empty list, single element, duplicates)") {
            val values = Arb.list(Arb.int(1..10), 0..3)
                .samples(RandomSource.seeded(1237L)).take(100).map { it.value }

            println(values)

            values.any { it.isEmpty() } shouldBe true          // empty list
            values.any { it.size == 1 } shouldBe true          // single element list
            values.any { it.distinct().size < it.size } shouldBe true // duplicates
        }
...
//        test("Arb.nullable(Int) edge case (null 포함)") {
//            val samples = Arb.int(1..10)
//                .nullable()
//                .samples(RandomSource.seeded(1239L)).take(100).map { it.value }
//            println(samples)
//            samples.any { it == null } shouldBe true
//        }
    }
